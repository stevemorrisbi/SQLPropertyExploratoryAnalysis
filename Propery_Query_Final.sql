...
SELECT *
FROM [Property2021].[dbo].[Property_Prices_2021]
WHERE column12 = 'CARDIFF'

-- Sales in CF83 ordered by Sale Date
SELECT column2 as Price, CAST(column3 AS Date) as Sale_Date
FROM [Property2021].[dbo].[Property_Prices_2021]
WHERE column4 LIKE 'CF83%'
ORDER BY column3

-- Days of the year where CF83 had more than 1 sale
SELECT column2 as Price, CAST(column3 AS Date) as Sale_Date, COUNT(*) as Sales
FROM [Property2021].[dbo].[Property_Prices_2021]
WHERE column4 LIKE 'CF83%'
...
SELECT column2 Sale_Price, column12 City, COUNT(*) Total_Sales
FROM [Property2021].[dbo].[Property_Prices_2021]
WHERE column12 = 'CARDIFF'
GROUP BY column2, column12
...
SELECT column2 Sale_Price, column12 City, COUNT(*) AS Total_Sales
FROM [Property2021].[dbo].[Property_Prices_2021]
WHERE column12 = 'CARDIFF'
GROUP BY column2, column12
HAVING COUNT(*) < 5
)
SELECT AVG(Sale_Price) AS avg_price_under5
FROM orders_under_5;
...
SELECT AVG(Sale_Price) AS avg_price_over5 
FROM orders_over_5 

-- Sales in CF83 between 1st February and 1st April ordered by Date
SELECT column2 as Price, CAST(column3 AS Date) as Sale_Date
FROM [Property2021].[dbo].[Property_Prices_2021]
WHERE column4 LIKE 'CF83%' AND column3 BETWEEN '2021-01-31' AND '2021-03-31'
ORDER BY column3

-- Putting Sales between 1st February and 1st April into Price Catergories using CASE statement
SELECT column2 AS Price,
	CASE WHEN column2 <150000 THEN 'Cheap'
		WHEN column2 <200000 THEN 'Medium Price'
		WHEN column2 <300000 THEN 'High Price'
		ELSE 'Expensive'
		END AS Price_Rating,
...
SELECT Location, Sale_Price,
	CASE WHEN Sale_Price <150000 THEN 'Cheap'
		WHEN Sale_Price <200000 THEN 'Medium Price'
		WHEN Sale_Price <300000 THEN 'High Price'
		ELSE 'Expensive'
		END AS Price_Rating
FROM price_cat
...
SELECT Location, COUNT(*)AS Total_Sales_Cat, Sale_Price,
	CASE WHEN Sale_Price <150000 THEN 'Cheap'
		WHEN Sale_Price <200000 THEN 'Medium Price'
		WHEN Sale_Price <300000 THEN 'High Price'
		ELSE 'Expensive'
		END AS Price_Rating, 
		DENSE_RANK() OVER(ORDER BY Sale_Price DESC) AS Rnk_Cat
FROM price_cat
GROUP BY Location, Sale_Price
ORDER BY Rnk_Cat 

-- Calculating the AVG 10% morgage price for properties (with Post Codes) in Cardiff
SELECT column12 AS Location, AVG(column2) /10 AS Min_Morgage 
FROM  [Property2021].[dbo].[Property_Prices_2021]
WHERE column12 = 'CARDIFF' AND  column4 IS NOT NULL
GROUP BY column12

-- Calculating the AVG 10% morgage price for properties (with Post Codes) in Caerphilly
SELECT column12 AS Location, AVG(column2) /10 AS Min_Morgage 
FROM  [Property2021].[dbo].[Property_Prices_2021]
WHERE column12 = 'CAERPHILLY' AND  column4 IS NOT NULL
GROUP BY column12

-- Total property sales by month during 2021 in Caerphilly
SELECT column12 AS Location, COUNT(DISTINCT column2) AS Property_Sales, (MONTH(column3)) AS Month_Number
FROM  [Property2021].[dbo].[Property_Prices_2021]
WHERE column12 = 'CAERPHILLY' AND  column4 IS NOT NULL
GROUP BY (MONTH(column3)), column12
...
SELECT column12 AS Location, COUNT(DISTINCT column2) AS Property_Sales, 
AVG(DISTINCT column2) Avg_Sale_Price, (MONTH(column3)) AS Month_Number
FROM  [Property2021].[dbo].[Property_Prices_2021]
WHERE column12 = 'CARDIFF' AND  column4 IS NOT NULL
GROUP BY (MONTH(column3)), column12
...
SELECT column12 AS Location, COUNT(DISTINCT column2) AS Property_Sales,
AVG(DISTINCT column2) Avg_Sale_Price, (MONTH(column3)) AS Month_Number,
MAX(DISTINCT column2) AS Max_Sale, MIN(DISTINCT column2) AS Min_Sale
FROM  [Property2021].[dbo].[Property_Prices_2021]
WHERE column12 = 'CAERPHILLY' AND  column4 IS NOT NULL
GROUP BY (MONTH(column3)), column12
ORDER BY Avg_Sale_Price DESC
...
SELECT column12 AS location, COUNT(*) count_above_avg
FROM [Property2021].[dbo].[Property_Prices_2021]
WHERE column12 = 'CARDIFF' AND column2 >( 
SELECT AVG(DISTINCT column2) Avg_Sale_Price
FROM [Property2021].[dbo].[Property_Prices_2021]
WHERE column12 = 'CARDIFF'
)
GROUP BY column12
...
SELECT column12 AS location, COUNT(*) count_below_avg
FROM [Property2021].[dbo].[Property_Prices_2021]
WHERE column12 = 'CARDIFF' AND column2 <( 
SELECT AVG(DISTINCT column2) Avg_Sale_Price
FROM [Property2021].[dbo].[Property_Prices_2021]
...
SELECT AVG(column2) avg_price, LEFT(column4,4) post_code, column12 location
FROM [Property2021].[dbo].[Property_Prices_2021]
WHERE column12 = 'CARDIFF'and column4 IS NOT NULL
GROUP BY column4, column12
ORDER BY avg_price DESC

-- Days of most sales for Caerphilly and Cardiff together
SELECT LEFT(column3,10) sale_date, count(3) as total_sales, sum(column2) as price_sum
from [Property2021].[dbo].[Property_Prices_2021]
WHERE column12 IN( 'CARDIFF', 'CAERPHILLY')
group by column3
order by total_sales desc 
...
WITH dateclean AS(
SELECT column2 Sale_Price, LEFT(column3,10) Sale_Date, column12 Sale_Location1,
LAG(column12) OVER (ORDER BY column3) as Sale_Location2,
LAG(column2) OVER (ORDER BY column3) as Previous_Sale_Price,
LAG(column3) OVER (ORDER BY column3) as Previous_Sale_Date
FROM [Property2021].[dbo].[Property_Prices_2021]
WHERE column12 IN( 'CARDIFF', 'CAERPHILLY')
)
SELECT Sale_Location1, Sale_Price, Sale_Date, Sale_Location2, Previous_Sale_Price, 
